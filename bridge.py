...
class VoiceAssistantBridge:
    ...
    def setup_mqtt(self):
        ...
        try:
            ...
            self.logger.info(f"Connected to MQTT broker at {Config.MQTT_HOST}:{Config.MQTT_PORT}")
...
"""
    ...
    def control_light(self, params: Dict[str, Any]):
        ...
        if action == 'turn_on':
            ...
            if brightness is not None:
                service_data["brightness"] = int(brightness * 255 / 100)  # Convert to 0-255
    ...
    def call_ha_service(self, domain: str, service: str, service_data: Dict[str, Any]):
        ...
        url = f"http://{Config.HA_HOST}:{Config.HA_PORT}/api/services/{domain}/{service}"
        ...
        self.logger.info(f"Calling HA service: {domain}.{service} with data: {service_data}")
